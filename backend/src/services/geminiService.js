 ...
 */
const FAQ_PATH = require('path').join(__dirname, '../../faq/pregnancy_faq.json');
let faqCache = null;
...
function loadFaq() {
  ...
  try {
    ...
    faqCache = [];
    logger.warn('pregnancy_faq.json not found — chatbot will run without FAQ context.');
  }
  return faqCache;
}
...
async function streamAnswer(userMessage, history = [], languageHint = 'en', res) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    logger.error('Gemini streaming API error', { status: response.status, body: errText });
