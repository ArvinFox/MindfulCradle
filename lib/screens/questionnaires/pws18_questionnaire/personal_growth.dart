...
class PersonalGrowthPage extends StatefulWidget {
  const PersonalGrowthPage({super.key});

  @override
  State<PersonalGrowthPage> createState() => _PersonalGrowthPageState();
}

class _PersonalGrowthPageState extends State<PersonalGrowthPage> {
  bool _started = false;
  ...
  Future<void> _loadJson(String lang) async {
    try {
      ...
      setState(() {
        final provider = Provider.of<PWS18Provider>(context, listen: false);
        final ids = provider.subscales['Personal Growth'] ?? [];
      ...
      if (kDebugMode) print("Personal Growth JSON load error: $e");
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _currentLang == 'en' ? 'Personal Growth' : 'පුද්ගලික වර්ධනය',
              ...
              onPressed: !_allAnswered(provider) || _isSubmitting
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        if (kDebugMode)
                          print("Error saving Personal Growth: $e");
                      } finally {
