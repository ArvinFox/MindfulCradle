...
router.get('/', async (req, res) => {
  const snap = await db.collection('videos').orderBy('sessionNumber', 'asc').get();
...
router.post(
  ...
  validateRequest,
  async (req, res) => {
    const { title, titleSi, youtubeId, youtubeIdSi, duration, sessionNumber } = req.body;
...
router.put(
  ...
  validateRequest,
  async (req, res) => {
    const ALLOWED = ['title', 'titleSi', 'youtubeId', 'youtubeIdSi', 'duration', 'sessionNumber'];
    const updates = {};
 ...
 */
router.delete('/:id', [param('id').isString().notEmpty()], validateRequest, async (req, res) => {
